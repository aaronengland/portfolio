...
def get_application_table(str_values, str_datecol, list_cols, unique_id, dict_list_errors):
    # convert str_values to a df
    X = pd.read_csv(
        StringIO(str_values), 
        delimiter=',', 
        #parse_dates=[str_datecol], 
        #usecols=list_cols,
    )
    # if X is empty
    if X.empty:
        ...
        # set date to today
        X[str_datecol] = dt.datetime.today()
    else:
        # lower
...
# get ln table
def get_lexis_nexis_table(str_values, list_cols, unique_id, dict_list_errors):
...
def helper_parse_tuxml(str_values, list_cols_raw_tu):
    # get root
    ...
    for child in root.iter(tag='{http://www.transunion.com/namespace}characteristic'):
        ...
        # logic
        if str_col_name in list_cols_raw_tu:
            # get col val
            try:
...
# get tu table
def get_transunion_table(str_values, list_cols, unique_id, dict_list_errors):
    # parse xml
    X = helper_parse_tuxml(
        str_values=str_values, 
        list_cols_raw_tu=list_cols,
    )
    # if empty
    if X.empty:
        # append error
