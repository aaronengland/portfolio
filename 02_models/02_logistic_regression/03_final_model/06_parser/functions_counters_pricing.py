...
class PricingCounters:
	...
	def map_to_tier(self):
		...
		# get current apr
		self.df[f'current_apr'] = self.df[f'tier'].map(self.dict_apr_current)
		# get roe required
		self.df[f'roe_required'] = self.df[f'tier'].map(self.dict_roe_required)
		# get discount 
		self.df[f'raw_discount'] = self.df[f'tier'].map(self.dict_discount)
		# get op ex
		self.df[f'operating_expenses'] = self.df[f'tier'].map(dict_op_ex)
		# get reserve points
		self.df[f'reserve'] = self.df[f'tier'].map(dict_reserve_points)
		# return
		return self
	# get equity
	def get_equity(self):
		self.df['equity'] = self.flt_equity_intercept + (self.df['ecnl'] * self.flt_equity_slope)
		# return
		return self
	# get profit
	def get_profit(self):
		self.df['profit'] = self.df['roe_required'] * self.df[f'equity']
		# return
		return self
	# get discount
	def get_discount(self):
		self.df['discount'] = self.df['raw_discount'] / self.df['amt_financed'] / self.flt_avg_life
		# return
		return self
	# get late fee
	...
	def get_raw_discount_adjustment(self):
		self.df['raw_discount_adjustment'] = self.df.apply(
			lambda x: get_raw_discount_adjustment(
				...
				flt_amt_financed=x['amt_financed'], 
	...
	def get_additional_discount_needed(self):
		self.df['additional_discount_needed'] = self.df['rate_cap_handicap'] * self.df['amt_financed'] * self.flt_avg_life
		# return
		return self
	# get apr
	def get_apr(self):
